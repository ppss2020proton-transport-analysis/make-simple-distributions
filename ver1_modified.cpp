...
void ProtonTransport::simple_tracking(double obs_point, const string& optics_file_name){
  ...
  optics_root_file_name += "p8_13TeV_p_100k_t_205m" + 
...
}   //here for loop for particle ends
...
int main() {
    std::string path_to_optic_files = "optics_PPSS_2020/";
    std::string optics_file = "alfaTwiss1.txt_beta40cm_6500GeV_y-185murad";
    std::string prepareBeamline_input=path_to_optic_files+=optics_file;

    ProtonTransport* p_default = new ProtonTransport;
    p_default->PrepareBeamline(prepareBeamline_input, false);
    p_default->simple_tracking(205., optics_file);
    delete p_default;
    
    
    std::vector<double> shift_values={0.0005,0.0002,0.0001,-0.0005,-0.0002,-0.0001};
    std::vector<double> shift_values_z={0.005,0.001,0.0001,-0.005,-0.001,-0.0001};
    

    // I really don't like the part below too, but for now it should work, and I need files for the plot code
    ...
    for (int i = 0; i < shift_values.size(); ++i)
    {
        ...
        p_shifted->PrepareBeamline(prepareBeamline_input, false);
        p_shifted->SetShift(Magnet{"quadrupole", j}, Shift{shift_values[i], 0, 0});
        p_shifted->simple_tracking(205., optics_file+"_quad_"+std::to_string(j)+"_x_"+std::to_string(shift_values[i]));
        delete p_shifted;
    }
        ...
        for (int i = 0; i < shift_values.size(); ++i)
    {
        ProtonTransport* p_shifted = new ProtonTransport;
        p_shifted->PrepareBeamline(prepareBeamline_input, false);
        p_shifted->SetShift(Magnet{"quadrupole", j}, Shift{0,shift_values[i], 0});
        p_shifted->simple_tracking(205., optics_file+"_quad_"+std::to_string(j)+"_y_"+std::to_string(shift_values[i]));
        delete p_shifted;
    }

        for (int i = 0; i < shift_values_z.size(); ++i)
    {
        ProtonTransport* p_shifted = new ProtonTransport;
        p_shifted->PrepareBeamline(prepareBeamline_input, false);
        p_shifted->SetShift(Magnet{"quadrupole", j}, Shift{0, 0,shift_values_z[i]});
        p_shifted->simple_tracking(205., optics_file+"_quad_"+std::to_string(j)+"_z_"+std::to_string(shift_values_z[i]));
        delete p_shifted;
    }
